...
from myForms import forms,modelForms
...
def signup(request):
    form = forms.FormSignUp(request.POST or request.GET or None )
...
def Book_Entry(request):
    form = modelForms.MyBooksForm(request.POST or request.GET or None)
    if form.is_valid():
        form.save(commit=True)
        return index(request)
    return render(request,'myFormsApp/bookEntry.html',context={'modelForm' : form})
